...
public class MessengerScreen extends Screen implements DefaultLifecycleObserver {
    ...
    protected MessengerScreen(@NonNull CarContext carContext, SpeechToTextSession speechToTextSession) {
        ...
        am = (AudioManager) this.carContext.getSystemService(Context.AUDIO_SERVICE);
    ...
    public Template onGetTemplate() {
        connection = this.speechToTextSession.connection;
        reply = new Messenger(this.speechToTextSession.getHandler());

        speechToTextSession.getServiceConnection();
        getButtonTexts();
...
//        if (!mBound) {
//            this.carContext.bindService(serviceIntent, connection, Context.BIND_AUTO_CREATE);
//        }
//    }

    private void getButtonTexts() {

        Log.println(Log.INFO, "xxx", "car getbuttontexts mbound:" + mBound);
        if (mBound) {
